...
class Lead(db.Model):
    ...
    photos = db.Column(db.Text, nullable=True) # Stores filenames
...
# ---------------------------------------------------------
# 3. ROBUST AUTO-MIGRATION SYSTEM
# ---------------------------------------------------------
with app.app_context():
    db.create_all()
    inspector = inspect(db.engine)
    
    # Ensure User Table is up to date
    ...
    # Ensure Lead Table has ALL new fields
    lead_columns = [c['name'] for c in inspector.get_columns('leads')]
    with db.engine.connect() as conn:
        columns_to_check = [
            ('year_built', 'INTEGER'), ('square_footage', 'INTEGER'), ('lot_size', 'TEXT'),
            ('bedrooms', 'INTEGER'), ('bathrooms', 'FLOAT'), ('hvac_type', 'TEXT'),
            ('hoa_fees', 'TEXT'), ('parking_type', 'TEXT'), ('emailed_count', 'INTEGER DEFAULT 0'),
            ('asking_price', 'TEXT'), ('photos', 'TEXT')
        ]
        for col_name, col_type in columns_to_check:
            if col_name not in lead_columns:
                try:
                    print(f"Migrating DB: Adding {col_name}...")
...
def page_not_found(e):
    return render_template('base.html', content='<div class="container mt-5 text-center"><h1>404</h1><p>The page you are looking for does not exist.</p><a href="/dashboard" class="btn btn-primary">Go to Dashboard</a></div>'), 404

@app.errorhandler(500)
def internal_server_error(e):
    return render_template('base.html', content='<div class="container mt-5 text-center"><h1>500</h1><p>Internal Server Error. Our team has been notified.</p></div>'), 500

@app.errorhandler(RequestEntityTooLarge)
def handle_file_too_large(e):
    flash("File is too large! Maximum limit is 16MB.", "error")
    return redirect(request.url)

# ---------------------------------------------------------
# 5. ACCESS CONTROL (Paywall + Admin)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 6. "VICIOUS" MULTI-SOURCE SCRAPER
# ---------------------------------------------------------
def search_off_market(city, state):
    ...
    print(f"--- STARTING VICIOUS SCAN FOR: {city}, {state} ---")
    
    # COMBINED POWER QUERIES
    # 1. Zillow Backdoor (Finds owner phones in descriptions)
    # 2. Gov Files (PDFs of vacant/eviction lists)
    # 3. Social Media (Facebook Marketplace/Groups leaks)
    # 4. Classifieds (Craigslist/FSBO)
    queries = [
        ...
        f'"{city}" "code enforcement" violations filetype:csv',
        f'"{city}" "tax sale" list filetype:xls',
        f'site:facebook.com "{city}" "selling my house" -group -marketplace',
        f'site:craigslist.org "{city}" "real estate" -agent phone',
        f'"{city}" "must sell" "cash only" "needs repairs"'
    ]
    
    leads_found = []
    service = build("customsearch", "v1", developerKey=SEARCH_API_KEY)

    # Randomize query order to avoid pattern detection
    random.shuffle(queries)

    # Run top 8 queries to maximize coverage
    for q in queries[:8]:
        try:
            ...
            for item in res.get('items', []):
                ...
                source_label = "Web"
                if "zillow" in item.get('link'): source_label = "Zillow Backdoor"
                elif "facebook" in item.get('link'): source_label = "Social Media"
                elif ".pdf" in item.get('link'): source_label = "Gov File"
                elif "craigslist" in item.get('link'): source_label = "Classifieds"
...
def dashboard():
    ...
    # 3. Admin: Fetch Seller Inbound Forms
    seller_submissions = []
...
@login_required
def add_manual_lead():
    """Manual Lead Entry for user found leads"""
    try:
        new_lead = Lead(
        ...
        flash(f"Error adding lead: {e}", "error")
...
def export_leads():
    ...
    si = io.StringIO()
    cw = csv.writer(si)
    cw.writerow(['Status', 'Address', 'Phone', 'Email', 'Source', 'Link', 'Notes'])
...
def hunt_leads():
    ...
    return jsonify({'message': f"Scan Complete. Found {count} new leads across the web."})
...
def send_emails_background(app, user_id, subject, body, attachment_path):
    with app.app_context():
        ...
        print(f"--- Starting Email Campaign for User {user_id} ---")
        
        count = 0
        for lead in leads:
            if lead.email and '@' in lead.email:
                print(f"Simulating email to {lead.email}...")
                # Simulate delay to prevent spam flagging
        ...
        # Cleanup attachment after blast
        if attachment_path and os.path.exists(attachment_path):
            os.remove(attachment_path)
        
        print(f"--- Campaign Finished. {count} emails sent. ---")
...
def email_campaign():
    ...
    thread.start()
    return jsonify({'message': "🚀 Campaign Started! Emails are being sent in the background."})
...
def create_video():
    ...
    if not HAS_FFMPEG:
        time.sleep(2)
        return jsonify({'video_url': "https://www.w3schools.com/html/mov_bbb.mp4", 'message': "AI Video Generated (Preview Only - No GPU)"})
...
def social_post():
    ...
    return jsonify({'message': f'🚀 Queued for upload to {platform} (Draft Mode). Check your mobile app!'})
...
<div class="modal fade" id="addLeadModal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header"><h5 class="modal-title">Add Lead Manually</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <form action="/leads/add" method="POST">
      <div class="modal-body">
        <div class="mb-2"><label>Address (Required)</label><input name="address" class="form-control" required></div>
        <div class="row">
            <div class="col-6 mb-2"><label>Phone</label><input name="phone" class="form-control"></div>
            <div class="col-6 mb-2"><label>Email</label><input name="email" class="form-control"></div>
        </div>
        <div class="row">
            <div class="col-4"><label>Beds</label><input name="bedrooms" type="number" class="form-control"></div>
            <div class="col-4"><label>Baths</label><input name="bathrooms" type="number" step="0.5" class="form-control"></div>
            <div class="col-4"><label>Price</label><input name="asking_price" class="form-control"></div>
        </div>
      </div>
      <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button><button type="submit" class="btn btn-primary">Save Lead</button></div>
      </form>
    </div>
  </div>
</div>
