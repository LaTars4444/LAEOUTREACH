...
const Hunter: React.FC = () => {
  const { addLead, addLog, user } = useStore();
  ...
  useEffect(() => {
    if (!user?.hasAiAccess) addLog("⛔ Access denied: AI module required.", "error");
  }, [user, addLog]);

  const addNotification = (msg: string) => {
    setNotifications(prev => [msg, ...prev].slice(0, 50)); // Keep last 50 notifications
  };
    ...
    try {
      ...
      if (data.leads?.length) {
        for (const lead of data.leads) {
          ...
          await new Promise(r => setTimeout(r, 200)); // Smooth UI updates
        }
      }
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-8 shadow-xl">
        ...
        <div className="mt-6 bg-slate-900 border border-slate-700 rounded p-4 max-h-80 overflow-y-auto">
          ...
          <ul className="text-white text-sm space-y-1">
            {progressLeads.map(l => (
              <li key={l.id}>{l.address} | {l.name} | {l.email || 'No Email'} | {l.phone || 'No Phone'}</li>
            ))}
          </ul>
        </div>

        {/* Notifications panel (replacing Terminal) */}
