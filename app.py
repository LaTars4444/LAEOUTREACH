base64, time, random, stripe, sqlite3, json
    ...
    <div class="w-full max-w-2xl bg-white rounded-[3rem] shadow-2xl p-8 md:p-16 border border-gray-100 mt-10">
        ...
        {% if step == 'gate' %}
            <div class="space-y-4">
                <a href="/outreach" class="block w-full p-10 border-4 border-black rounded-[2.5rem] hover:bg-black hover:text-white transition-all group">
                    ...
                    <p class="mt-4 text-xs font-bold opacity-60 italic">Campaign Manager</p>
                </a>
                <a href="/market-choice" class="block w-full p-10 bg-blue-600 text-white rounded-[2.5rem] hover:bg-blue-700 transition-all shadow-xl">
                    ...
                    <p class="mt-4 text-xs font-bold opacity-80 italic">Buy/Sell Properties</p>
                </a>
            </div>
        ...
        {% elif step == 'market-choice' %}
            ...
            <div class="grid grid-cols-1 gap-4 text-center">
                ...
                <a href="/auth/login?next=buy" class="p-8 border-2 border-black rounded-[2rem] hover:bg-gray-50 transition">
                    <span class="block text-[10px] font-black uppercase text-gray-400 mb-2 italic">Buyer</span>
                    <h4 class="text-2xl font-black italic uppercase leading-none">Set Buy-Box & View Leads</h4>
                </a>
            </div>

        {% elif step == 'buybox-setup' %}
            <div class="mb-8"><h2 class="text-3xl font-black italic uppercase">Configure Buy-Box</h2><p class="text-gray-400 text-xs font-bold uppercase">Define your deployment capacity</p></div>
            <form action="/save-buybox" method="POST" class="space-y-6">
                <div class="space-y-4">
                    <label class="block p-6 border-2 rounded-3xl cursor-pointer hover:border-blue-600 transition">
                        <input type="radio" name="bb_type" value="any" class="mr-2" checked>
                        <span class="font-black italic uppercase">Option 1: Receive Everything</span>
                        <p class="text-[10px] text-gray-400 ml-6 uppercase">I want to see every off-market lead, regardless of price.</p>
                    </label>
                    <label class="block p-6 border-2 rounded-3xl cursor-pointer hover:border-blue-600 transition">
                        <input type="radio" name="bb_type" value="budget" class="mr-2">
                        <span class="font-black italic uppercase">Option 2: Specific Budget</span>
                        <input type="number" name="budget" placeholder="Max Price (USD)" class="block w-full mt-4 border-b-2 border-black p-2 outline-none text-xl font-black italic">
                    </label>
                </div>
        ...
        {% elif step == 'buyer-feed' %}
            <div class="flex justify-between items-center mb-8"><a href="/" class="text-xs font-black uppercase underline">← Menu</a><a href="/buybox-reset" class="text-[10px] font-black uppercase text-gray-400">Edit Buy-Box</a></div>
            <div class="bg-blue-50 p-4 rounded-2xl mb-6 flex justify-between items-center border border-blue-100">
                <span class="text-[10px] font-black uppercase text-blue-600 italic">Targeting: {{ bb_label }}</span>
            ...
            <div class="space-y-4">
                ...
                <div class="p-8 border-2 border-black rounded-[2rem] flex flex-col md:flex-row justify-between items-center hover:bg-gray-50 transition">
                    <div class="mb-4 md:mb-0">
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Lead #{{ lead.id }}</p>
                        <h5 class="text-xl font-black italic uppercase leading-tight">Property Assignment</h5>
                ...
                {% else %}
                <div class="text-center py-20 italic font-black text-gray-200 text-3xl">NO MATCHES FOUND</div>
                {% endfor %}
            </div>
        {% elif step == 'seller-form' %}
            ...
            <form action="/submit-lead" method="POST" class="space-y-4">
                ...
                <textarea name="reason" placeholder="Property Details & Reason for selling..." class="w-full border-2 p-5 rounded-2xl h-32 outline-none" required></textarea>
                <button class="w-full bg-blue-600 text-white py-6 rounded-[2.5rem] font-black text-2xl italic shadow-xl">Get Offer Intro</button>
            </form>
        {% endif %}
        ...
        <div class="mt-20 pt-10 border-t">
            ...
            <div class="h-32 overflow-y-scroll p-4 bg-gray-50 rounded-2xl text-[9px] leading-relaxed text-gray-400 border font-medium">
                <p><strong>ZERO LIABILITY:</strong> You agree to HOLD HARMLESS the platform from any losses or disputes. We are a tech provider, not a broker. All assignment fees are for software access only. AES-256 encryption is active for privacy.</p>
            </div>
        </div>
    </div>
</body>
...
def submit_lead():
    ...
    val = int(request.form.get('valuation', 0))
    ...
    with get_db() as conn:
        ...
        conn.commit()
    flash("Submitted! AI is matching you with Buyers now.")
