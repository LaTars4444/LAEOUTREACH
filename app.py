...
# ---------------------------------------------------------
# REMODELED: Using indexed placeholders {0} {1} only. 
# This prevents the 'manual field specification' Render crash.
# ---------------------------------------------------------
SYSTEM_LOGS = []
...
def log_activity(message):
    ...
    try:
        ...
        # --- THE SURGICAL REMODEL OF LINE 53 ---
        log_template = "[{0}] {1}"
...
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'titan_v1_industrial_auth_v10')
app.config['MAX_CONTENT_LENGTH'] = 64 * 1024 * 1024 # 64MB Buffer

# DB PERSISTENCE (Support for Render Persistent Storage)
...
try:
    ...
    groq_client = None
    log_activity("⚠️ AI WARNING: GROQ_API_KEY missing. Fallback active.")
...
try:
    ...
    log_activity("⚠️ FFMPEG WARNING: Video rendering using simulation.")
...
class User(UserMixin, db.Model):
    ...
    # Universal Templates
    email_template = db.Column(db.Text, default="Hi [[NAME]], I am interested in your property at [[ADDRESS]]. I am a cash buyer and can close quickly. Please let me know if you are interested.")
...
# --- INDUSTRIAL AUTO-MIGRATOR (FIXES 500 ERRORS) ---
with app.app_context():
    ...
    if 'email_template' not in user_cols:
        with db.engine.connect() as conn:
            ...
            conn.commit()
            log_activity("⚙️ DB REPAIR: Added email_template column.")
    ...
    if 'name' not in lead_cols:
        with db.engine.connect() as conn:
            ...
            conn.commit()
            log_activity("⚙️ DB REPAIR: Added name column to leads.")
...
def task_scraper(app_obj, user_id, city, state):
    ...
    - Randomized Human Behavior (5-15s Sleep).
    - Corrected indexed query construction.
    """
                        ...
                        for item in res.get('items', []):
                            ...
                            if phones or emails:
                                if not Lead.query.filter_by(link=link, submitter_id=user_id).first():
                                    lead = Lead(submitter_id=user_id, address=item.get('title')[:100], phone=phones[0] if phones else "None", email=emails[0] if emails else "None", source="{0} ({1})".format(site, kw), link=link)
                                    db.session.add(lead)
                                    total_leads_added += 1
...
def task_emailer(app_obj, user_id, subject, body, attach_path):
    ...
    - Constant Universal Scripts with dynamic address/name injection.
    ...
    with app_obj.app_context():
        ...
        log_activity("📧 BLAST STARTING: Targeting {0} potential deals.".format(len(leads)))
            ...
            for lead in leads:
                try:
                    ...
                    if groq_client and len(final_body) < 10:
                        chat = groq_client.chat.completions.create(messages=[{"role": "user", "content": "Write a short cash offer for {0}.".format(lead.address)}], model="llama-3.3-70b-versatile")
...
def register():
    if request.method == 'POST':
        if not User.query.filter_by(email=request.form['email']).first():
            user = User(email=request.form['email'], password=generate_password_hash(request.form['password'], method='scrypt'))
...
<div class="row g-4">
 ...
 <div class="col-12">
  <ul class="nav nav-tabs mb-4" id="titanTab" role="tablist">
   ...
   <li class="nav-item"><button class="nav-link" id="hunter-tab" data-bs-toggle="tab" data-bs-target="#hunter">🕵️ Hunter</button></li>
  ...
  <div class="tab-content">
   ...
   <div class="tab-pane fade" id="email"><div class="card shadow-sm border-primary"><div class="card-header bg-primary text-white fw-bold d-flex justify-content-between"><span>📧 Universal Outreach Config</span><small>Use [[ADDRESS]] and [[NAME]] tags</small></div><div class="card-body"><form action="/email/template/save" method="POST" class="mb-4"><textarea name="template" class="form-control mb-2" rows="4">{{ user.email_template }}</textarea><button class="btn btn-sm btn-primary">Save Template</button></form><hr>{% if not gmail_connected %}<div class="alert alert-danger">Connect Gmail App Password in Settings!</div>{% endif %}<div class="mb-3"><label class="form-label fw-bold">Email Subject</label><input id="emailSubject" class="form-control" value="Regarding property at [[ADDRESS]]"></div><button onclick="sendBlast()" class="btn btn-dark w-100 fw-bold" {% if not gmail_connected %}disabled{% endif %}>🚀 Launch Outreach Mission</button></div></div></div>
   <div class="tab-pane fade" id="history"><div class="card shadow-sm"><div class="card-body"><h5 class="fw-bold">Outreach History</h5><div class="list-group">{% for item in history %}<div class="list-group-item"><h6 class="mb-1 fw-bold">{{ item.address }}</h6><p class="mb-1 small">{{ item.message }}...</p><small class="text-primary">Sent to: {{ item.recipient }} | {{ item.sent_at.strftime('%m-%d %H:%M') }}</small></div>{% endfor %}</div></div></div></div>
  </div>
 </div>
</div>
...
<script>
const usData = {"AL":["Birmingham","Montgomery","Mobile"],"AK":["Anchorage"],"AZ":["Phoenix","Tucson"],"AR":["Little Rock"],"CA":["Los Angeles","San Diego","San Francisco","Sacramento"],"CO":["Denver"],"CT":["Hartford"],"DE":["Wilmington"],"FL":["Miami","Tampa","Orlando"],"GA":["Atlanta"],"HI":["Honolulu"],"ID":["Boise"],"IL":["Chicago"],"IN":["Indianapolis"],"IA":["Des Moines"],"KS":["Wichita"],"KY":["Louisville"],"LA":["New Orleans"],"ME":["Portland"],"MD":["Baltimore"],"MA":["Boston"],"MI":["Detroit"],"MN":["Minneapolis"],"MS":["Jackson"],"MO":["St. Louis"],"MT":["Billings"],"NE":["Omaha"],"NV":["Las Vegas"],"NH":["Manchester"],"NJ":["Newark"],"NM":["Albuquerque"],"NY":["New York City","Buffalo"],"NC":["Charlotte"],"ND":["Fargo"],"OH":["Columbus","Cleveland"],"OK":["Oklahoma City"],"OR":["Portland"],"PA":["Philadelphia"],"RI":["Providence"],"SC":["Charleston"],"SD":["Sioux Falls"],"TN":["Nashville"],"TX":["Houston","Dallas","Austin"],"UT":["Salt Lake City"],"VT":["Burlington"],"VA":["Virginia Beach"],"WA":["Seattle"],"WV":["Charleston"],"WI":["Milwaukee"],"WY":["Cheyenne"]};
